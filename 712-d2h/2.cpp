    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = i; j < n && s[j] - s[i] <= R; ++j) {
            for (int k = MAXN - 1; k >= 0; --k) if (seg_cnt[k]) {
                ...
                sum_sqr[k + 1] += sum_sqr[k] + 2.0 * s[j] * sum[k + 1] + (double)s[j] * s[j] * seg_cnt[k];
                sum[k + 1] += sum[k] + (double)s[j] * seg_cnt[k];

                subset_cnt += seg_cnt[k];
                var_sum += (sqr_sum[k] + (double)s[j] * s[j] * seg_cnt[k] + (2.0 * s[j] * sum[k + 1] + (double)s[j] * s[j] * seg_cnt[k]) * (-2.0 / (k + 1) + 1.0 / (k + 1) / (k + 1))) / (k + 1);
            }
        }
    }
